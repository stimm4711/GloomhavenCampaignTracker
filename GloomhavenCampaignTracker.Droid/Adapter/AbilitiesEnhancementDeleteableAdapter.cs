...
namespace GloomhavenCampaignTracker.Droid.Adapter
{
    ...
    {
        private readonly DL_CharacterAbility _ability;
        private readonly Context _context;
        private readonly bool _isTop;
        private readonly List<DL_CharacterAbilityEnhancement> _enhancements;

        public override int Count => _enhancements.Count; 

        public AbilitiesEnhancementDeleteableAdapter(Context context, DL_CharacterAbility ability, bool istop)
        {
            _ability = ability;
            _context = context;
            _isTop = istop;

            if(_ability == null ||_ability.AbilityEnhancements == null)
            {
                _enhancements = new List<DL_CharacterAbilityEnhancement>();
            }
            else
            {
                _enhancements = _ability.AbilityEnhancements.Where(x => x.IsTop == istop).ToList();
            }            
        }
        ...
        private void ConfirmDeleteDialog(int position)
        {
            ...
            new CustomDialogBuilder(_context, Resource.Style.MyDialogTheme)
                ...
                .SetPositiveButton(_context.Resources.GetString(Resource.String.YesDelete), (senderAlert, args) =>
                {
                    ...
                    _ability?.AbilityEnhancements.Remove(item);
        ...
        /// Adapter for Character abilities
        /// </summary>
        internal class AbilitiesEnhancementDeleteableAdapterOld : BaseAdapter
        {
            ...
            public AbilitiesEnhancementDeleteableAdapterOld(Context context, DL_Ability ability, bool istop)
            {
                _ability = ability;
                _context = context;
                _isTop = istop;
