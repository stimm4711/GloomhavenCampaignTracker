...
namespace GloomhavenCampaignTracker.Shared.Business
{
    ...
    public class CampaignUnlockedScenario : Java.Lang.Object
    {
        ...
        internal HashSet<CampaignUnlockedScenario> SetIncomplete(HashSet<CampaignUnlockedScenario> removededScenarios = null)
        {
            if(removededScenarios == null) removededScenarios = new HashSet<CampaignUnlockedScenario>();

            if (!Completed) return removededScenarios;

            var unlockedScenarioNumbers = GetUnlockedScenarios();
            ...
            {
                if (GCTContext.CurrentCampaign.UnlockedScenarios.Any(x => x.Completed && 
                                                                        x.Scenarionumber != Scenarionumber &&
                                                                        !removededScenarios.Contains(x) &&
                                                                        x.GetUnlockedScenarios().Contains(scenarioNumber))) continue;
                
                var campScenario = currentCampaign.UnlockedScenarios.FirstOrDefault(x => x.Scenarionumber == scenarioNumber);
                
                if (campScenario != null)
                {
                    removededScenarios.Add(campScenario);

                    foreach (var cus in campScenario.SetIncomplete(removededScenarios))
                    {
